...
if(GLOBALS->traces.first)
	...
	if((GLOBALS->is_ghw)&&(0))
		...
		{
		fileselbox("Write LXT File As",&GLOBALS->filesel_lxt_writesave,G_CALLBACK(menu_write_lxt_file_cleanup), G_CALLBACK(NULL),"*.lxt", 1);
		}
	}
	else
	{
...
if(GLOBALS->traces.first)
	{
	fileselbox("Write VCD File As",&GLOBALS->filesel_vcd_writesave,G_CALLBACK(menu_write_vcd_file_cleanup), G_CALLBACK(NULL),"*.vcd", 1);
	}
	else
	{
	status_text("No traces onscreen to save!\n");
...
if(GLOBALS->traces.first)
	{
	fileselbox("Write TIM File As",&GLOBALS->filesel_tim_writesave,G_CALLBACK(menu_write_tim_file_cleanup), G_CALLBACK(NULL),"*.tim", 1);
	}
	else
	{
	status_text("No traces onscreen to save!\n");
  ...
  if(dirty)
    {
      ...
      entrybox("Create Group",300,"","Enter group name:",128,G_CALLBACK(create_group_cleanup));
    }
  else
    {
      must_sel_bg();
...
if(!mnv && !GLOBALS->busy_busy_c_1)
	...
	if(GLOBALS->helpbox_is_active)
		...
		fileselbox("Select a trace to view...",&GLOBALS->filesel_newviewer_menu_c_1,G_CALLBACK(menu_new_viewer_cleanup), G_CALLBACK(NULL), NULL, 0);
		}
	mnv = 0;
	}
}
...
if((!GLOBALS->socket_xid)&&(!GLOBALS->partial_vcd))
	{
	fileselbox("Select a trace to view...",&GLOBALS->filesel_newviewer_menu_c_1,G_CALLBACK(menu_new_viewer_tab_cleanup), G_CALLBACK(NULL), NULL, 0);
	}
}
...
while(t)
	...
	if((t->flags&TR_HIGHLIGHT)&&(!(t->flags&(TR_BLANK|TR_ANALOG_BLANK_STRETCH)))&&(t->name))
		{
		showchange("Show-Change All", GLOBALS->showchangeall_menu_c_1=t, G_CALLBACK(menu_showchangeall_cleanup));
		return;
		}
	t=t->t_next;
	}
...
entrybox("Regexp UnHighlight",300,GLOBALS->regexp_string_menu_c_1,NULL,128,G_CALLBACK(regexp_unhighlight_cleanup));
}
/**/
static void
...
entrybox("Regexp Highlight",300,GLOBALS->regexp_string_menu_c_1,NULL,128,G_CALLBACK(regexp_highlight_cleanup));
}
...
errno = 0;
fileselbox("Grab To File",&GLOBALS->filesel_imagegrab,G_CALLBACK(menu_write_screengrab_cleanup), G_CALLBACK(NULL), "*.png", 1);
}
...
fileselbox("Read Save File",&GLOBALS->filesel_writesave,G_CALLBACK(menu_read_save_cleanup), G_CALLBACK(NULL), GLOBALS->is_gtkw_save_file ? "*.gtkw" : "*.sav", 0);
}
...
if(!stems_are_active())
	...
	if(GLOBALS->stems_type != WAVE_ANNO_NONE)
		{
		fileselbox("Read Verilog Stemsfile",&GLOBALS->stems_name, G_CALLBACK(menu_read_stems_cleanup), G_CALLBACK(NULL), NULL, 0);
		}
		else
		{
...
fileselbox("Read Logfile",&GLOBALS->filesel_logfile_menu_c_1,G_CALLBACK(menu_read_log_cleanup), G_CALLBACK(NULL), NULL, 0);
}
...
fileselbox("Read Script File",&GLOBALS->filesel_scriptfile_menu,G_CALLBACK(menu_read_script_cleanup), G_CALLBACK(NULL), "*.tcl", 0);
}
